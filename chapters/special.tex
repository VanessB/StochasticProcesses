...
\begin{definition}[Явная конструкция пуассоновского процесса]
    ...
    $ S_n = \xi_1 + \ldots + \xi_n $.
    Тогда процесс $ K_t = \sup \{ n \mid S_n \leqslant t \} $ называется \defemph{пуассоновским процессом с интенсивностью $ \lambda $}.
\end{definition}

Процесс $ K_t $, построенный по случайным величинам $ \xi_k $ способом, указанным выше,
...
\[
    \proba \{ \xi_i = t \} = \lim_{\Delta t \to +0} \left( 1 - \lambda \Delta t \right)^{\frac{t}{\Delta t}} = e^{-\lambda t}
\]
...
\subsection{Винеровский процесс} \label{subsection:Wiener}

Винеровский процесс, как и пуассоновский, также имеет множество применений;
он описывает симметричное случайное блуждание, непрерывное во времени.
Например, речь может идти о броуновском движении,
или о динамике цен акций (за вычетом тренда).
